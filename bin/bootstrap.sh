...
function ensure_essentials() {
  ...
  if is_linux; then
    ...
    if [ "${PACKAGE_MANAGER}" == "apt" ]; then
      sudo apt-get update
      sudo apt-get install -y jq apt-transport-https gnupg2 curl nginx apache2-utils pv unzip dialog software-properties-common gettext-base iputils-ping wireguard-tools
    fi
...
if [ -f "$HOME/config.yaml" ]; then
  ...
  if grep -Fxq "# Values for Playground One." $HOME/config.yaml; then
    echo 'found config.yaml in home'
    mv $HOME/config.yaml $ONEPATH
  else
    echo 'run pgo -c'
  fi
fi
