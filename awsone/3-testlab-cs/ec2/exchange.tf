...
# Exchange
# #############################################################################
resource "aws_network_interface" "exchange_eni" {
  subnet_id       = var.private_subnets[0]
  private_ips     = [var.exchange_private_ip]
...
resource "aws_instance" "exchange" {
  ...
  ami                         = var.ami_exchange != "" ? var.ami_exchange : data.aws_ami.windows.id
  instance_type               = var.exchange_instance_type
  iam_instance_profile        = var.ec2_profile
  key_name                    = var.key_name
  user_data                   = local.userdata_exchange
  get_password_data           = false
  user_data_replace_on_change = true

  network_interface {
    network_interface_id = aws_network_interface.exchange_eni.id
    device_index         = 0 # Primary network interface
  }

  root_block_device {
    volume_size           = var.exchange_root_volume_size
    volume_type           = var.exchange_root_volume_type
    delete_on_termination = true
    encrypted             = true
  }

  tags = {
    Name          = "${var.environment}-exchange"
