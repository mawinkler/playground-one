...
output "public_instance_ip_windows1" {
  value = length(aws_instance.windows1) > 0 ? aws_instance.windows1[0].public_ip : ""
}
...
output "ssh_instance_windows1" {
  ...
  value       = length(aws_instance.windows1) > 0 ? "ssh -i ${var.private_key_path} -o StrictHostKeyChecking=no ${var.windows_username}@${aws_instance.windows1[0].public_ip}" : ""
}
