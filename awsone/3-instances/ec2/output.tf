...
#
output "linux_ip" {
  value = length(aws_instance.linux-server) > 0 && var.vpn_gateway == false ? [for i in range(length(aws_instance.linux-server)) : aws_instance.linux-server[i].public_ip] : null
}
...
#
output "windows_ip" {
  value = length(aws_instance.windows-server) > 0 && var.vpn_gateway == false ? [for i in range(length(aws_instance.windows-server)) : aws_instance.windows-server[i].public_ip] : null
}
